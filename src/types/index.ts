...
/** Kurum detayında «Kurum genel bilgileri»: 8 kart; `title` platform sabitleriyle doldurulur, yalnızca `body` düzenlenir. */
export interface InstitutionAboutCard {
  title: string;
  body: string;
}

/** Program modalındaki tek satır: başlık + alt metin (şeffaf kutu içinde). */
export interface InstitutionProgramModalItem {
  title: string;
  subtitle: string;
}
...
export interface InstitutionProgramCard {
  ...
  body: string;
  modalItems: InstitutionProgramModalItem[];
}
...
export interface Institution {
  ...
  /** Detayda «Kurum genel bilgileri» altında 8 kart; başlıklar sabit, yalnızca gövde metni düzenlenir. */
